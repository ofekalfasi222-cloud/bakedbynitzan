...
function sanitizeUrl(url) {
  ...
  if (trimmed.startsWith('https://') || trimmed.startsWith('http://') || trimmed.startsWith('tel:') || trimmed.startsWith('mailto:')) return trimmed;
  return '';
}
...
function formatPriceHTML(product, size = 'normal') {
  ...
  const note = product.priceNote
    ? `<span style="${noteStyle}">(${escHtml(product.priceNote)})</span>`
    : '';
...
function createProductCard(product) {
  ...
  card.innerHTML = `
    ...
    ${product.badge ? `<span class="product-card-badge">${escHtml(product.badge)}</span>` : ''}
    <div class="product-card-content">
      <div class="product-card-category">${escHtml(product.categoryLabel)}</div>
      <h3 class="product-card-title">${escHtml(product.name)}</h3>
      <p class="product-card-description">${escHtml(product.description)}</p>
      <div class="product-card-footer">
        <span class="product-card-price">${formatPriceHTML(product)}</span>
        <a href="${createWhatsAppLink(product.name)}" class="product-card-order" target="_blank" rel="noopener">
          להזמנה
        </a>
      </div>
    </div>
  `;
  card.querySelector('.product-card-order').addEventListener('click', function(e) {
    e.stopPropagation();
    typeof gtag==='function'&&gtag('event','whatsapp_order',{product_name:product.name});
  });
...
function openMobileViewer(product) {
  ...
  const mvOrderBtn = viewer.querySelector('.mv-order-btn');
  if (mvOrderBtn) mvOrderBtn.addEventListener('click', function() {
    typeof gtag==='function'&&gtag('event','whatsapp_order',{product_name:product.name,source:'mobile'});
  });
...
function applySiteSettings() {
  ...
  if (s.instagramUrl) {
    const safeIgUrl = sanitizeUrl(s.instagramUrl);
    if (safeIgUrl) document.querySelectorAll('.ig-link').forEach(el => el.href = safeIgUrl);
  }
    ...
    if (faqList) {
      ...
      s.faq.forEach(item => {
        ...
        div.innerHTML = `
          ...
          <div class="faq-answer"><p>${escHtml(item.a)}</p></div>
        `;
  ...
  if (s.footerText) { const el = document.getElementById('footerText'); if (el) el.textContent = s.footerText; }
}
